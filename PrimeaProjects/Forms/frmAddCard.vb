...
Public Class frmAddCard
    ...
    Private Sub txtInfoUpdateEntry(ByVal info As Object)
        ...
        If mjrec.ControllerSN > 0 Then
            ...
            Dim Ccode As String = Regex.Replace(reRep, "[^0-9]", "")

            txtCode.Text = Ccode

            '    Dim hexCcode As String = Hex(Val(Ccode))

            '  MsgBox("Hexa CardCode : " & hexCcode, vbInformation)

            'Dim CcodeL As Integer = Len(hexCcode)
            ' Dim ccCodeMid As Integer = CcodeL

            ' Dim Enc1 As String = Mid(hexCcode, 1, 2)
            ' Dim Enc2 As String = Mid(hexCcode, ccCodeMid)
