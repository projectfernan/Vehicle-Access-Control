...
Public Class controlDev
    ...
    Function ViewIn(ByVal Ccode As String) As Boolean
        ...
        Try
            ...
            rs = New Recordset
            rs = Conn.Execute("select CardCode,PlateNo,Name,DateExpired from vweinfo where CardCode = '" & Ccode & "'")
            If rs.EOF = False Then
                ...
                If dtNow >= dtExp Then
                    frmSystemMain.fillLogs(C_Name, C_Plate, Now, "(In)Expired")
                ...
                End If
            Else
                frmSystemMain.fillLogs(Ccode, "Unknown", Now, "(In)UnknownCard")
                frmSystemMain.txtMsg.Text = "UNKNOWN CARD"
                Return False
            End If
    ...
    Function ViewOut(ByVal Ccode As String) As Boolean
        ...
        Try
            ...
            If rs.EOF = False Then
                ...
                Dim dtExp As Date = rs("DateExpired").Value
                If dtNow >= dtExp Then
                    frmSystemMain.fillLogs(C_Name2, C_Plate2, Now, "(Out)Expired")
                    frmSystemMain.txtMsg.Text = "CARD ALREADY EXPIRED"
                    Return False
                Else
                    Return True
                End If
            Else
                frmSystemMain.fillLogs(Ccode, "Unknown", Now, "(Out)UnknownCard")
                frmSystemMain.txtMsg.Text = "UNKNOWN CARD"
                Return False
            End If
        Catch ex As Exception
    ...
    Sub saveIn()
        ...
        Try
            ...
            frmSystemMain.fillLogs(C_Name, C_Plate, Now, "In")
        Catch ex As Exception
    ...
    Sub saveInQuery()
        ...
        Try
            Dim rs As New Recordset
            Dim strTmeIn As String = Format(Now, "yyyy-MM-dd HH:mm:ss")
            rs = New Recordset
            rs = Conn.Execute("insert into tbltimein(CardCode,Plate,TimeIn)VALUES('" & C_Code & "','" & C_Plate & "','" & strTmeIn & "')")
    ...
    Sub saveOut()
        ...
        Try
            ...
            frmSystemMain.fillLogs(C_Name2, C_Plate2, Now, "Out")
        Catch ex As Exception
            frmSystemMain.txtMsg.Text = ex.Message
        End Try
    End Sub
    ...
    Sub saveOutQuery()
        ...
        Try
            Dim rs As New Recordset
            rs = New Recordset
            Dim strTmeOut As String = Format(Now, "yyyy-MM-dd HH:mm:ss")
            rs = Conn.Execute("insert into tbltimeout(CardCode,Plate,TimeOut)VALUES('" & C_Code2 & "','" & C_Plate2 & "','" & strTmeOut & "')")
    ...
    Sub saveInOutQuery(ByVal Ccode As String)
        If conServer() = False Then
            frmSystemMain.txtMsg.Text = "Server disconnected"
            Exit Sub
        End If
        ...
        Try
            ...
            If rs1.EOF = False Then
                ...
                'C_Plate2 = "FFF111"
                rs = New Recordset
                Dim strdtIn As String = Format(CDate(rs1("TimeIn").Value), "yyyy-MM-dd HH:mm:ss")
                Dim strDtNow As String = Format(Now, "yyy-MM-dd HH:mm:ss")
                rs = Conn.Execute("insert into tblinout(CardCode,Plate,TimeIn,TimeOut)VALUES('" & C_Code2 & "','" & C_Plate2 & "','" & strdtIn & "','" & strDtNow & "')")
                ...
                Case strOut
                    ...
                    If PassbackStat() = True Then
                        ...
                        Else
                            ...
                            saveInOutQuery(txtCardCode.Text)
    ...
    Public Function Cam_Connect2(ByVal strIP As String, ByVal IntPort As Integer, ByVal strUID As String, ByVal strPWD As String, ByVal CamID As Integer) As Boolean
        With NetVid2
            Cam_Exit2()
            If PingMe(strIP) = False Then Return False
            Cam_ID = .Login(strIP, IntPort, strUID, strPWD)
    ...
    Public Function Cam_Capture2() As Boolean
        Delete_Image()
        With NetVid2
            Dim Cap As Integer = ctrlCap2
            If .JPEGCapturePicture(Cap, 0, 0, Application.StartupPath & "\Capture") = True Then
                Return True
            Else
                Return False
    ...
    Private Sub controller_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If ctrlNode = vbNullString Then
            ...
            Cam_Connect2(ctrlCamIP, ctrlCamPort, ctrlCamUID, ctrl_CamPWD, ctrlCam2)
    ...
    Private Sub ConnectedToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ConnectedToolStripMenuItem.Click
        ...
        With frmSystemMain
            saveAudiUpdate(.UserAcc.Text, .SystemTag.Text, "Connecting to : " & txtCtrl.Text & " Controller")
        End With
    ...
    Private Sub RefreshCam1ToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RefreshCam1ToolStripMenuItem.Click
        With frmSystemMain
            saveAudiUpdate(.UserAcc.Text, .SystemTag.Text, "Disconnecting to : " & txtCtrl.Text & " Controller")
        End With
    ...
    Private Sub DisconnectToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DisconnectToolStripMenuItem.Click
        ...
        With frmSystemMain
            saveAudiUpdate(.UserAcc.Text, .SystemTag.Text, "Refresh Camera 1 of : " & txtCtrl.Text & " Controller")
        End With
    End Sub
    ...
    Private Sub ConnectToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ConnectToolStripMenuItem.Click
        If Cam_Connect2(ctrlCamIP, ctrlCamPort, ctrlCamUID, ctrl_CamPWD, ctrlCam2) = True Then
            MsgBox("Camera is active! ", vbInformation, "Refresh Camera 2")
        Else
            MsgBox("Failed to connect in Camera 2! ", vbExclamation, "Refresh Camera 2")
        End If

        With frmSystemMain
            saveAudiUpdate(.UserAcc.Text, .SystemTag.Text, "Refresh Camera 2 of : " & txtCtrl.Text & " Controller")
        End With
    ...
    Private Sub TestTriggerToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TestTriggerToolStripMenuItem.Click
        ...
        With frmSystemMain
            saveAudiUpdate(.UserAcc.Text, .SystemTag.Text, "Test Triggered the : " & txtCtrl.Text & " Controller")
        End With
    ...
    Private Sub NormallyCloseToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles NormallyCloseToolStripMenuItem.Click
        Dim controlConfigure As New wgMjControllerConfigure
